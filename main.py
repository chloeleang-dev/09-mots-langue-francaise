...
def read_data(filename):
    ...
    with open(filename, mode = 'r', encoding='utf8') as f:
        s = f.readlines()
        l = [elt.strip() for elt in s] # strip pour retirer les /n et tout les truc invisibles
...
def cherche1(mots, start, stop, n):
    ...
    ensemblerecherche1 = set()
    for mot in mots:
        if mot.startswith(str(start)) and mot.endswith(str(stop)) and len(mot) == n:
            ensemblerecherche1.add(mot)

    return ensemblerecherche1
...
def cherche2(mots, lstart, lmid, lstop, nmin, nmax):
    ...
    Returns:
        ...
        par une chaine présente dans lstart, contenant une 
        chaine présente dans lmid et finissant par une chaine 
    ...
    {'alphabétisassiez'}
    """
    print(lstart, lstop, lmid, nmin, nmax)
    ensemblerecherche2 = set()
    for mot in mots:

        # print(mot)

        longueur = False
        if  len(mot) >= nmin and len(mot) <= nmax:
            longueur = True
        if not longueur:
            continue
        # print(f"{longueur=}")
        ...
        ensemblerecherche2.add(mot)

    return ensemblerecherche2

def main():
    """main"""
    # mots = read_data(FILENAME)
    # ens = ensemble_mots(FILENAME)
    # print( [mot for mot in ["chronophage",
    # "procrastinateur", "dangerosité", "gratifiant"] if mot in ens])
    ...
    # # mab17ez = cherche2(mots, 'a', 'b', 'z', 16, 16)
    # # print(mab17ez)
    # res = cherche2(ens, ['c'], ['h', 'l', 'o'], ['e'], 5, 6)
    # print(res)
    # print(len(res))
